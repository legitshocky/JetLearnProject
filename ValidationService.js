function validateInput(data, requiredFields) {
  if (!data) return { isValid: false, message: "No data provided." };
...
function validateMigrationData(data) {
  ...
  if (!data.jlid || data.jlid.trim() === '') errors.push('JLID is required');
  if (!data.learner || data.learner.trim() === '') errors.push('Learner name is required');
  if (!data.newTeacher || data.newTeacher.trim() === '') errors.push('New teacher is required');
  ...
  if (!data.clsManager || data.clsManager.trim() === '') errors.push('CLS Manager is required');
  if (!data.jetGuide || data.jetGuide.trim() === '') errors.push('JetGuide is required');
  if (!data.startDate || data.startDate.trim() === '') errors.push('Start Date is required');
