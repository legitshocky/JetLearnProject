...
/**
 * Adds a note engagement to a HubSpot DEAL (not ticket).
 * Used for attrition notes, class credit notes, etc.
 */
function addNoteToHubSpotDeal(dealId, noteBody) {
  try {
    var token = PropertiesService.getScriptProperties().getProperty('HUBSPOT_API_KEY');
...
/**
 * Reverse-lookup: display name → HubSpot internal ID from TEACHER_HS_DATA sheet.
 * Col 1 = internal ID, Col 2 = display name.
 */
function _getTeacherInternalId(displayName) {
  ...
  } catch(e) { Logger.log('[_getTeacherInternalId] ' + e.message); }
  return null;
}

/**
 * Assigns a new teacher to the latest migration ticket for a JLID.
 * Called from Teacher Persona "Migrate to" button.
 * Returns { success, ticketId, teacherName }
 */
function assignTeacherToTicket(jlid, teacherName) {
  ...
  if (!ticketResult.found) return { success: false, message: 'No open migration ticket found for ' + jlid + '. Create one in HubSpot first.' };

  var ticketId = ticketResult.ticketId;
  var token    = PropertiesService.getScriptProperties().getProperty('HUBSPOT_API_KEY');

  // Prefer internal HubSpot ID; fall back to display name if not in sheet
  var valueToSet = _getTeacherInternalId(teacherName) || teacherName;
  Logger.log('[assignTeacher] ' + jlid + ' → ticket ' + ticketId + ' new_teacher=' + valueToSet);

  var resp = monitoredFetch('https://api.hubapi.com/crm/v3/objects/tickets/' + ticketId, {
    method: 'PATCH',
    headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' },
    payload: JSON.stringify({ properties: { new_teacher: valueToSet } }),
    muteHttpExceptions: true
  });

  var code = resp.getResponseCode();
  Logger.log('[assignTeacher] PATCH → HTTP ' + code);
...
// ─────────────────────────────────────────────────────────────────────────────
// TASK QUEUE
// Fetches open HubSpot tasks for Sourav (owner 61546090), enriches with
// associated deal JLID + learner name, and categorises for 1-click execution.
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Resolves the HubSpot owner ID for a given email via the Owners API.
 * Result cached in ScriptProperties for 12 hours to avoid repeated lookups.
 * Returns null if not found.
 */
function _resolveHubSpotOwnerIdByEmail(email, token) {
  var cacheKey = 'hs_owner_id_' + email.replace(/[^a-z0-9]/gi, '_');
  var cached = PropertiesService.getScriptProperties().getProperty(cacheKey);
  if (cached) { Logger.log('[_resolveOwner] cached id=' + cached); return cached; }
  ...
  try {
    ...
    if (res.getResponseCode() === 200) {
      var data = JSON.parse(res.getContentText());
      var owner = data.results && data.results[0];
      if (owner && owner.id) {
        PropertiesService.getScriptProperties().setProperty(cacheKey, String(owner.id));
        Logger.log('[_resolveOwner] resolved ' + email + ' → id=' + owner.id);
        return String(owner.id);
      }
    }
  } catch(e) { Logger.log('[_resolveOwner] err: ' + e.message); }
  return null;
}

/**
 * Returns all open HubSpot tasks assigned to the current user (looked up by email),
 * enriched with JLID + learner name from associated deals.
 */
function getMyHubSpotTasks() {
  try {
    var token = PropertiesService.getScriptProperties().getProperty('HUBSPOT_API_KEY');
    if (!token) return { success: false, error: 'No HubSpot API key' };

    var ownerId = '594932888'; // Sourav Pal — CRM owner ID (confirmed)

    // 1. Search open tasks for this owner
    var searchUrl = 'https://api.hubapi.com/crm/v3/objects/tasks/search';
    var filters = [
    ...
    var body = {
      ...
      properties: [
        'hs_task_subject', 'hs_task_status', 'hs_task_type',
        'hs_timestamp', 'hs_task_body', 'hubspot_owner_id'
      ],
      sorts: [{ propertyName: 'hs_timestamp', direction: 'DESCENDING' }],
      limit: 100
    };

    var res = monitoredFetch(searchUrl, {
      method: 'post',
      headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' },
      payload: JSON.stringify(body),
      muteHttpExceptions: true
    ...
    if (res.getResponseCode() !== 200) {
      return { success: false, error: 'HubSpot API error ' + res.getResponseCode() };
    }

    var data    = JSON.parse(res.getContentText());
    var results = data.results || [];
    Logger.log('[getMyHubSpotTasks] task count=' + results.length);

    // 2a. Batch-fetch deal associations for all task IDs
    var taskDealMap   = {}; // taskId -> dealId
    var taskTicketMap = {}; // taskId -> ticketId
    if (results.length) {
      var taskInputs = results.map(function(t) { return { id: t.id }; });

      // Deal associations
      try {
        var assocRes = monitoredFetch('https://api.hubapi.com/crm/v4/associations/tasks/deals/batch/read', {
          method: 'post',
          headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' },
          payload: JSON.stringify({ inputs: taskInputs }),
          muteHttpExceptions: true
        });
        if (assocRes.getResponseCode() === 200) {
          (JSON.parse(assocRes.getContentText()).results || []).forEach(function(r) {
            if (r.from && r.to && r.to.length) taskDealMap[r.from.id] = String(r.to[0].toObjectId || r.to[0].id);
          });
        }
        Logger.log('[getMyHubSpotTasks] task->deal map size=' + Object.keys(taskDealMap).length);
      } catch(ae) { Logger.log('[getMyHubSpotTasks] deal assoc err: ' + ae.message); }

      // 2b. Ticket associations
      try {
        var tickAssocRes = monitoredFetch('https://api.hubapi.com/crm/v4/associations/tasks/tickets/batch/read', {
          method: 'post',
          headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' },
          payload: JSON.stringify({ inputs: taskInputs }),
          muteHttpExceptions: true
        });
        if (tickAssocRes.getResponseCode() === 200) {
          (JSON.parse(tickAssocRes.getContentText()).results || []).forEach(function(r) {
            if (r.from && r.to && r.to.length) taskTicketMap[r.from.id] = String(r.to[0].toObjectId || r.to[0].id);
          });
        }
        Logger.log('[getMyHubSpotTasks] task->ticket map size=' + Object.keys(taskTicketMap).length);
      } catch(te) { Logger.log('[getMyHubSpotTasks] ticket assoc err: ' + te.message); }
    }

    // 3. Batch-fetch deals to get JLID + learner name
    var dealMap = {};
    var dealIdSet = {};
    Object.keys(taskDealMap).forEach(function(k) { dealIdSet[taskDealMap[k]] = true; });
    var dealIds = Object.keys(dealIdSet);

    if (dealIds.length) {
      try {
        var batchRes = monitoredFetch('https://api.hubapi.com/crm/v3/objects/deals/batch/read', {
          method: 'post',
          headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' },
          payload: JSON.stringify({ inputs: dealIds.map(function(id) { return { id: id }; }), properties: ['jetlearner_id', 'dealname'] }),
          muteHttpExceptions: true
        });
        if (batchRes.getResponseCode() === 200) {
          (JSON.parse(batchRes.getContentText()).results || []).forEach(function(d) {
            dealMap[d.id] = { jlid: d.properties.jetlearner_id || '', learnerName: d.properties.dealname || '' };
          });
        }
      } catch(be) { Logger.log('[getMyHubSpotTasks] deal batch err: ' + be.message); }
    }

    // 3b. Batch-fetch tickets to get subject/name
    var ticketMap = {};
    var ticketIdSet = {};
    Object.keys(taskTicketMap).forEach(function(k) { ticketIdSet[taskTicketMap[k]] = true; });
    var ticketIds = Object.keys(ticketIdSet);

    if (ticketIds.length) {
      try {
        var tBatchRes = monitoredFetch('https://api.hubapi.com/crm/v3/objects/tickets/batch/read', {
          method: 'post',
          headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' },
          payload: JSON.stringify({ inputs: ticketIds.map(function(id) { return { id: id }; }), properties: ['subject', 'hs_ticket_id'] }),
          muteHttpExceptions: true
        });
        if (tBatchRes.getResponseCode() === 200) {
          (JSON.parse(tBatchRes.getContentText()).results || []).forEach(function(tk) {
            ticketMap[tk.id] = { subject: tk.properties.subject || ('Ticket #' + tk.id), ticketId: tk.id };
          });
        }
      } catch(tbe) { Logger.log('[getMyHubSpotTasks] ticket batch err: ' + tbe.message); }
    }

    // 4. Build enriched task list
    var now   = new Date();
    var tasks = results.map(function(t) {
      var props    = t.properties || {};
      var subject  = (props.hs_task_subject || '').trim();
      var dueRaw   = props.hs_timestamp || '';
      var dueDate  = dueRaw ? new Date(dueRaw) : null;
      var overdue  = dueDate && dueDate < now;
      var isToday  = dueDate && dueDate.toDateString() === now.toDateString();

      var jlid = '', learnerName = '', dealName = '';
      var dealId = taskDealMap[t.id];
      if (dealId && dealMap[dealId]) {
        jlid        = dealMap[dealId].jlid;
        learnerName = dealMap[dealId].learnerName;
        dealName    = dealMap[dealId].learnerName; // dealname IS learner name in this CRM
      }

      var ticketId = '', ticketName = '';
      var tId = taskTicketMap[t.id];
      if (tId && ticketMap[tId]) {
        ticketId   = ticketMap[tId].ticketId;
        ticketName = ticketMap[tId].subject;
      }
      ...
      return {
        ...
        dueDate:     dueRaw,
        dueDateFmt:  dueDate ? _fmtTaskDate(dueDate) : '—',
        overdue:     overdue && !isToday,
        dueToday:    isToday,
        jlid:        jlid,
        ...
        category:    _categoriseTask(subject)
      };
    });

    // Sort: overdue first, then today, then future
    tasks.sort(function(a, b) {
      var wa = a.overdue ? 0 : (a.dueToday ? 1 : 2);
      var wb = b.overdue ? 0 : (b.dueToday ? 1 : 2);
      if (wa !== wb) return wa - wb;
      return (a.dueDate || '').localeCompare(b.dueDate || '');
    });

    return { success: true, tasks: tasks, total: tasks.length };

  } catch(e) {
    Logger.log('[getMyHubSpotTasks] Error: ' + e.message);
 ...
 */
function completeHubSpotTask(taskId) {
  try {
    var token = PropertiesService.getScriptProperties().getProperty('HUBSPOT_API_KEY');
    var res = monitoredFetch('https://api.hubapi.com/crm/v3/objects/tasks/' + taskId, {
      method: 'patch',
      headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' },
      payload: JSON.stringify({ properties: { hs_task_status: 'COMPLETED' } }),
      muteHttpExceptions: true
    });
...
/**
 * Full task detail: properties + associated notes (comments).
 */
function getTaskDetails(taskId) {
  try {
    ...
    // 1. Task properties
    var taskRes = UrlFetchApp.fetch(
      base + '/crm/v3/objects/tasks/' + taskId +
      '?properties=hs_task_subject,hs_task_status,hs_task_type,hs_timestamp,hs_task_body,hubspot_owner_id,hs_task_priority,hs_queue_membership_ids',
      { headers: { 'Authorization': 'Bearer ' + token }, muteHttpExceptions: true }
    );
    if (taskRes.getResponseCode() !== 200) return { success: false, error: 'Task fetch failed ' + taskRes.getResponseCode() };
    var task = JSON.parse(taskRes.getContentText());
    var props = task.properties || {};

    // 2. Associated notes (comments)
    var comments = [];
    try {
      var assocRes = UrlFetchApp.fetch(
        base + '/crm/v4/objects/tasks/' + taskId + '/associations/notes',
        { headers: { 'Authorization': 'Bearer ' + token }, muteHttpExceptions: true }
      );
      if (assocRes.getResponseCode() === 200) {
        var noteIds = (JSON.parse(assocRes.getContentText()).results || []).map(function(r) { return r.toObjectId || r.id; });
        if (noteIds.length) {
          var noteBatch = UrlFetchApp.fetch(base + '/crm/v3/objects/notes/batch/read', {
            method: 'post',
            headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' },
            payload: JSON.stringify({ inputs: noteIds.map(function(id){ return {id:id}; }), properties: ['hs_note_body','hs_createdate','hubspot_owner_id'] }),
            muteHttpExceptions: true
          });
          if (noteBatch.getResponseCode() === 200) {
            comments = (JSON.parse(noteBatch.getContentText()).results || [])
              .sort(function(a,b){ return (a.properties.hs_createdate||'').localeCompare(b.properties.hs_createdate||''); })
              .map(function(n) {
                return {
                  noteId:    n.id,
    ...
    } catch(ce) { Logger.log('[getTaskDetails] notes err: ' + ce.message); }

    return {
      success:   true,
      taskId:    taskId,
      subject:   props.hs_task_subject  || '',
      body:      (props.hs_task_body    || '').replace(/<[^>]+>/g,' ').trim(),
      status:    props.hs_task_status   || '',
      type:      props.hs_task_type     || 'TODO',
...
function addTaskComment(taskId, commentText) {
  try {
    ...
    // 1. Create note
    var noteRes = UrlFetchApp.fetch(base + '/crm/v3/objects/notes', {
      method: 'post',
      headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' },
      payload: JSON.stringify({ properties: {
        hs_note_body:  commentText,
        hs_timestamp:  String(Date.now()),
        hubspot_owner_id: '594932888'
      }}),
      muteHttpExceptions: true
    });
    if (noteRes.getResponseCode() !== 201) return { success: false, error: 'Note create failed ' + noteRes.getResponseCode() };
    var noteId = JSON.parse(noteRes.getContentText()).id;

    // 2. Associate note → task
    UrlFetchApp.fetch(base + '/crm/v4/associations/notes/tasks/batch/create', {
      method: 'post',
      headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' },
      payload: JSON.stringify({ inputs: [{ from: { id: noteId }, to: { id: taskId } }] }),
      muteHttpExceptions: true
    });

    return { success: true, noteId: noteId, createdAt: new Date().toISOString(), body: commentText };
  } catch(e) {
    return { success: false, error: e.message };
  }
}
...
/**
 * Update task properties (due date, notes body, etc.)
 */
function updateTaskProperties(taskId, propsToUpdate) {
  try {
    var token = PropertiesService.getScriptProperties().getProperty('HUBSPOT_API_KEY');
    var res = UrlFetchApp.fetch('https://api.hubapi.com/crm/v3/objects/tasks/' + taskId, {
      method: 'patch',
      headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' },
      payload: JSON.stringify({ properties: propsToUpdate }),
      muteHttpExceptions: true
    });
    return { success: res.getResponseCode() === 200 };
  } catch(e) {
...
/** Classify task by subject line pattern → drives action button in UI */
function _categoriseTask(subject) {
  var s = subject.toLowerCase();
  if (s.indexOf('installation email') !== -1 || s.indexOf('install email') !== -1) return 'installation_email';
  if (s.indexOf('material requirement') !== -1)  return 'material_email';
  if (s.indexOf('credentials') !== -1)            return 'credentials';
  if (s.indexOf('certificate') !== -1)            return 'certificate';
  if (s.indexOf('afa') !== -1)                    return 'afa';
  if (s.indexOf('send kit') !== -1)               return 'kit';
  if (s.indexOf('execute the migration') !== -1 ||
      s.indexOf('execute migration') !== -1)      return 'migration';
  return 'manual';
}

function _fmtTaskDate(d) {
